#######################################################
###### Import packages, functions, and BCI data #######
#######################################################
...
#################################################
###### Calculate the KSDE of the BCI data #######
#################################################
...
band.range = seq(0.5, 0.8, .01) # Specify the bandwidth candidate to compute
cv = bandwidth.cv.fun(ppp = X, inten.formula = m.covariate,
                      data.covariate = bci.covars, band.range = band.range,
                      endpt = 1.5, equal = T, a = a)
# Plot the spectral diverenge and select the bandwidth = 0.62
...
for (k in seq_along(period.smooth.list.Re)){
  if (cate.comb[k, 1] != cate.comb[k, 2]){ # For the cross cases, plot the coherence
  ...
  }else{ # For marginal cases, just plot the KSDE
...
### Plot figure 2
cairo_pdf("figure_2.pdf", width = 8.25, height = 5, bg = "transparent")
...
# Plot the marginal KSDE and coherence
...
# Plot the pair correlation functions
for (k in seq_along(period.smooth.list.Re)){
  par(mar = c(1.8,1.75,1.9,1.3))
