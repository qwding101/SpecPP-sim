...
model = "M3"
sidelength = A1 = A2 = 10 # Correspond to pp.data[[1]]
...
pp.data = readRDS("../../../data/sim/M3/M3_A10-20-40.rds")
pp.data = pp.data[[1]] # Only select A = 10
...
result.11 = parallel.period.fun(i=1, j=1, pp.data, inten.fun.sep="est", endpt = 1.5)
result.22 = parallel.period.fun(i=2, j=2, pp.data, inten.fun.sep="est", endpt = 1.5)
...
### Bandwidth
band.fixed = rep(0.5, length(pp.data))
...
### Bandwidth selection
band.range = seq(0.48, 3.3, 0.025)
cl = makeCluster(detectCores() - 1)
registerDoParallel(cl)
band.cv = foreach(q = 1:length(pp.data),
                  .combine = 'c',
                  .packages = 'spatstat') %dopar%{
                    cv = bandwidth.cv.fun.sim2(ppp = pp.data[[q]], band.range = band.range)
