...
smooth.fun = function(w, period.mat, w.k1, w.k2, b1=1, b2=b1, loo = F){
  ...
  kernel.product = t(outer(X = w1.range, Y = w2.range, KK.fun,
...
plot.pairs.fun = function(period.smooth.mat.list, ppp, xnorm = T, freq.list = NULL, shared.legend = T,
                      remove = NULL){
...
period.fun.sim = function(i, j, ppp, inten.fun.sep="est", a = 0.025,
  ...
  int.prod = function(k, w_q, w_p, a, A1, A2, inten.fun.sep){
    int.fun = Vectorize( # Vectorize the argument w
      function(k, w, a, A, x_i){
        real.part = integrate(function(x, a, A, x_i, k, w) Re(ha.fun(x,a)*inten.fun.sep[[k]](x,x_i)*exp(-1i*x*A*w)),
                         lower=-1/2, upper=1/2, a=a, A=A, x_i=x_i, k=k, w=w)$value
        imag.part = integrate(function(x, a, A, x_i, k, w) Im(ha.fun(x,a)*inten.fun.sep[[k]](x,x_i)*exp(-1i*x*A*w)),
                         lower=-1/2, upper=1/2, a=a, A=A, x_i=x_i, k=k, w=w)$value
        return(real.part + 1i*imag.part)
      }
    )
    return(area(ppp.li[[k]])*int.fun(k, w_p, a, A=A1, x_i=1)*int.fun(k, w_q, a, A=A2, x_i=2))
  }
  
  Jh.fun = function(k){
    
    const = ((2*pi)^(-2/2))*(area(ppp.li[[k]])^(-1/2))/(1+a*(5/(4*pi^2) - 4/3))
    V1 = outer(freq.list$omega1, ppp.li[[k]]$x, exp.term, a=a, A=A1)
    V2 = outer(freq.list$omega2, ppp.li[[k]]$y, exp.term, a=a, A=A2)
    J_h.woconst = V2 %*% t(V1)
    
    if (is.null(inten.fun.sep)){
      return(const*J_h.woconst)
    }else{
      if (is.list(inten.fun.sep)){ # Use population intensity (scaled to [-0.5,0.5]^2 input)
        if (is.numeric(inten.fun.sep[[k]])){
          lambda = inten.fun.sep[[k]]
          stopifnot("The population intensity should be larger than zero."= lambda > 0)
          inten.fun.sep[[k]] = function(x, x_i){ 
            if (x_i == 1) return(lambda) 
            if (x_i == 2) return(1) 
          }
        }else if (!is.function(inten.fun.sep[[k]])){
        ...
        inten.fun.sep = list()
        coef.est = ppm(ppp.li[[k]] ~ I(x^2) + I(y^2))$coef
        inten.fun.sep[[k]] = function(x, x_i){ 
          if (x_i == 1) return(exp(coef.est[1] + coef.est[2]*(A1^2)*(x^2))) 
          if (x_i == 2) return(exp(coef.est[3]*(A2^2)*(x^2)))}
     }else if (inten.fun.sep == "~1"){
        inten.fun.sep = list()
        inten.fun.sep[[k]] = function(x, x_i){
          if (x_i == 1) return(intensity.ppp(ppp.li[[k]]))
          if (x_i == 2) return(1)}
     }
      
      mat.left = outer(freq.list$omega2, freq.list$omega1[freq.list$omega1 < 0], int.prod,
                       k=k, a=a, A1=A1, A2=A2, inten.fun.sep=inten.fun.sep)
      mat.center = outer(freq.list$omega2, 0, int.prod,
                         k=k, a=a, A1=A1, A2=A2, inten.fun.sep=inten.fun.sep)
      mat.right = Conj(matrix(rev(as.vector(mat.left)),
                              ncol = ncol(mat.left), nrow = nrow(mat.left)))
  ...
  if (i == j){
    ...
    period.mat = Re(Jh.fun(i)*Conj(Jh.fun(j)))
...
bandwidth.cv.fun.sim = function(ppp, inten.fun.sep = "est", a = 0.025,
  ...
  for (r in 1:nrow(cate.comb)){
    ...
    period.mat.ext.list[[r]] = Re(DFT.list[[cate.comb[r,1]]]*Conj(DFT.list[[cate.comb[r,2]]]))
  ...
  if (length(cate) == 1){ # For univariate process, no need to use parallel computing
    ...
    period.smooth.mat.list = foreach(r = 1:nrow(cate.comb),
                                     .export = c("Bartlett.fun","KK.fun","smooth.fun")) %dopar% {
                                       # Calculate the periodogram
                                       period.mat.ext = Re(DFT.list[[cate.comb[r,1]]]*Conj(DFT.list[[cate.comb[r,2]]]))
                                       # Calculate the kernel smoothed spectral estimator
                                       val = apply(omega.comb, MARGIN = 1, smooth.fun,
                                                   period.mat = period.mat.ext,
                                                   w.k1 = freq.ext.list$omega1, w.k2 = freq.ext.list$omega2,
